...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($code)) {
        ...
        $stmt = $conn->prepare("SELECT verification_code FROM donors WHERE email = :email");
        ...
        if (!$donor) {
            ...
            $messageClass = 'error';
        } elseif ($donor['verification_code'] == $code) {
            $stmt = $conn->prepare("UPDATE donors SET is_verified = 1, verification_code = NULL WHERE email = :email");
            $stmt->execute([':email' => $email]);

            $message = '✅ Your account has been successfully verified! <a href="../signin.php" class="link">You can now log in</a>.';
...
// Page title
$conf['page_title'] = 'BloodBank | Verify Donor Account';
...
<div class="form-page">
    <div class="form-container verify-container">
        <h2>Verify Your Donor Account</h2>
        ...
        <p class="note">Didn't get the code? <a href="donor_signup.php?email=<?php echo urlencode($email); ?>">Resend</a></p>
    </div>
</div>
