...
try { $rq=$conn->prepare("SELECT id, blood_type, units_needed AS units, urgency, status, created_at FROM blood_requests WHERE hospital_id=:hid ORDER BY created_at DESC LIMIT 8"); $rq->execute([':hid'=>$hospital_id]); $requests=$rq->fetchAll(PDO::FETCH_ASSOC);} catch(Exception $e){$requests=[];}
